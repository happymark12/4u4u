...
.jssort01 .c {
	...
	background: url('${pageContext.request.contextPath}/img/t01.png')
		-800px -800px no-repeat;
	_background: none;
}
						...
						<div class="aa-properties-details">
							...
							<div class="aa-properties-info">
									...
									<div id="jssor_1"
										...
										<div data-u="slides"
											...
											<c:forEach var='imgName' varStatus='vs' items='${images}'>
												<c:if test='${vs.first}'>
													<div data-p="144.50">
														<img data-u="image" src="/disksource/roomRentAd/${imgName}" /> <img
											...
											<c:forEach var='imgName' varStatus='vs' items='${images}'>
												<c:if test='${vs.first}'>
													<div data-p="144.50">
														<img data-u="image" src="/disksource/wantedRoomAd/${imgName}" /> <img
								...
								<c:if test="${empty images }">
									<div id="jssor_1"
										...
										<div data-u="slides"
											style="cursor: default; position: relative; top: 0px; left: 0px; width: 800px; height: 356px; overflow: hidden;">
													<div data-p="144.50">
														<img data-u="image" src="${pageContext.request.contextPath}/img/NoImage.png" /> <img
															data-u="thumb" src="${pageContext.request.contextPath}/img/NoImage.png" />
													</div>
								...
								<c:if test="${!empty roomRentAd }">
									...
									<a id="iframeControl" href="#iframeControl"><span
									...
									<hr style="height: 10px">
									<h3>物件類型 : ${roomRentAd.adRentType==0?"整層出租":(roomRentAd.adRentType==1?"獨立套房":(roomRentAd.adRentType==2?"分租套房":"出租房間(有公共空間)"))}</h3>
									
									<c:if test="${roomRentAd.adRentType==0}">
					             	<h5>總租金$${roomRentAd.adRentPrice} &nbsp;押金: ${roomRentAd.adDeposit==2?"2個月":(roomRentAd.adDeposit==1?"1個月":"面議")}
					             	</h5>
					             	<h5>
					             	格局:<c:if test="${roomRentAd.adRoomNum!=0}">${roomRentAd.adRoomNum}房</c:if><c:if test="${roomRentAd.adToiletNum!=0}">${roomRentAd.adToiletNum}衛</c:if><c:if test="${roomRentAd.adLivingRoomNum!=0}">${roomRentAd.adLivingRoomNum}廳</c:if><c:if test="${roomRentAd.adBalconyNum!=0}">${roomRentAd.adBalconyNum}陽台</c:if>
					             	</h5>
					             	<hr style="height: 10px">
									</c:if>
									<c:if test="${roomRentAd.adRentType!=0}">
									<c:forEach var='room' varStatus='vs' items='${roomRentAd.roomItems}'>
										<c:if test="${room.roomState==true}">
										<h5>$${room.rentPrice}${room.roomType==0?" (套房)":" (雅房)"} 押金: ${room.deposit==2?"2個月":(room.deposit==1?"1個月":"面議")}</h5>
										</c:if>
							        ...
							        <c:if test="${! empty roomRentAd.adExtraCost || ! empty agentFee }">
									...
									<c:forEach var='room' varStatus='vs' items='${roomRentAd.roomItems}'>
										<c:if test="${room.roomState==true}">	
											...
											<h5>房間${vs.count} &nbsp;$${room.rentPrice}${room.roomType==0?" (套房)":" (雅房)"}</h5>
											<ul>
													<c:if test="${! empty room.rentFloor && ! empty room.rentTotalFloor}">
														<li><h5>樓層: ${room.rentFloor}/${room.rentTotalFloor} </h5></li>
													</c:if>
									...
									<hr style="height: 10px">
									<c:if test="${roomRentAd.adRentType=='3'&&roomRentAd.adCurrentPeopleNum!='0'}">	
									<h3>當前室友狀況</h3>
									...
									<div class="row">
										...
										<div class="col-md-4 col-sm-3 col-xs-6">
											...
											<c:if test="${! empty roomRentAd.adFutureCoupleAccept && roomRentAd.adFutureCoupleAccept}">
												<h5>可</h5>
											</c:if>
											<c:if test="${! empty roomRentAd.adFutureCoupleAccept && !roomRentAd.adFutureCoupleAccept}">
												<h5>不可</h5>
									...
									<c:if test="${roomRentAd.adRentType==0 && !empty potentialList}">
									<br>
									<h3>可能的合租者</h3>
									<div class="row">
									<c:forEach var="candidate" items="${potentialList}">
									<div class=" col-sm-2 col-xs-3 align-center" >
											<a href="<c:url value='/_4u4u/findRoomDetail/?adStyle=1&adId=${candidate.get(2)}'/>" >
								...
								<c:if test="${!empty findRoomAd }">
									...
									<c:forEach var="candidate" items="${potentialList}">
									<div class=" col-sm-2 col-xs-3 align-center" >
											<a href="<c:url value='/_4u4u/roomRentDetail?adStyle=0&adId=${candidate.get(0)}'/>" >
											<img height='80' width='80' 
												src='${candidate.get(1)}'>
											<br>
								...
								<div class="row">
									<div class="col-xs-6 ">
										<h5><a id="showInterest" href="#">感興趣</a></h5>
									</div>
