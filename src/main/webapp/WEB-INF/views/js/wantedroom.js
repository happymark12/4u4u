		...
		function() {
			...
			$('#severalSuites').change(
					function() {
						...
						console.log('severalSuites'
								+ $('#severalSuites').attr('required'))
						console.log('severalRooms'
								+ $('#severalRooms').attr('required'))
					})
			...
			$('select[name="county"]').change(
					function() {
						...
						$('select[name="district"]').children().each(
								function(index) {
									if (index == 0) {
										return;
									} else {
										$('#districtSpan').append(
												`<input type="checkbox" name="districtList" value="`
														+ $(this).val()
														+ `"><span>`
														+ $(this).val()
														+ `</span>`);
		...
		.ready(
				function() {
					...
					$('#theFile')
							.change(
									function(e) {
										...
										uploadImageCount = (fileArray.length > imageCount) ? imageCount
												: (fileArray.length);
										for (let i = 0; i < uploadImageCount; i++) {
											let readFile = new FileReader();
											readFile
													.readAsDataURL(fileArray[i]);
					...
					function parseFile(e) {
						...
						let idName = 'Image_'
								+ (new Date().getTime() - (Math.floor((Math
										.random() * 10000 + 1))));

						$('#dropzone').append(
								`<div><button id="` + idName
...
if ($("#peopleNum_gender").val() == '1男1女' || $("#peopleNum_gender").val() == '2男'
		|| $("#peopleNum_gender").val() == '2女') {
	$("#ageDiv").hide();
	$("#ageRange").show();
} else {
	$("#ageDiv").show();
