...
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/updateWantedroom.js"></script>
<script
...
<body style="background: #F0F0F0;">
	...
	<p>&nbsp;</p>
	<form
		action="/4u4u/_4u4u/UpdateWantedRoomAdServlet?adStyle=${adStyle}&adId=${findRoomAd.findRoomId}"
			...
			<table class="PostAd">
					...
					<td>
						<fieldset>
							...
							<c:if test="${! empty peopleNumGender}">
								<select name="peopleNum_gender" id="peopleNum_gender"
									onchange="selectGenderToAge(this[selectedIndex].value)"
									required>
							...
							<c:if test="${! empty suiteQuantity || ! empty roomQuantity}">
								<select name="severalSuites" id="severalSuites" disabled>
                                  ...
                                  <select name="severalRooms"
									id="severalRooms" disabled>
							...
							<c:if test="${! empty agreeShare}">
								<c:if test="${agreeShare == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" id="agreeShare" name="agreeShare"
										value="agree" checked>願意
                                  	</c:if>
							...
							<div id="addressPart" class="form-row city-selector-set">
								想要居住的地區 : <label for="inputCity">縣市</label> <select
									class="county" name="county"></select>
								<!-- <label for="inputDistrict">鄉鎮區</label> -->
								<select class=" district" name="district" style="display: none"></select><br>
								<br id="districtBr"> <span id="districtSpan"> </span>
							</div>
							<br> 租金預算: <input id="budget" name="budget" type="number"
								min="0" max="100000000" required value="${budget}">元/月<br>
							<br> 可入住日期: <input type="date" name="checkInDate"
								id="checkInDate" value="${checkInDate}"><br>
							<br> 居住時間:
							...
							<c:if test="${! empty washMachine}">
								<c:if test="${washMachine == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasWashingMachine" value="true"
										checked>洗衣機
                                  	</c:if>
								<c:if test="${washMachine == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasWashingMachine" value="true">洗衣機
                                  	</c:if>
							</c:if>
							<c:if test="${! empty refrigerator}">
								<c:if test="${refrigerator == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasRefrigerator" value="true"
										checked>冰箱
                                  	</c:if>
								<c:if test="${refrigerator == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasRefrigerator" value="true">冰箱
                                  	</c:if>
							</c:if>
							<c:if test="${! empty TV}">
								<c:if test="${TV == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasTV" value="true" checked>電視
                                  	</c:if>
								<c:if test="${TV == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasTV" value="true">電視
                                  	</c:if>
							</c:if>
							<c:if test="${! empty airConditioning}">
								<c:if test="${airConditioning == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasAirConditioning" value="true"
										checked>冷氣
                                  	</c:if>
								<c:if test="${airConditioning == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasAirConditioning" value="true">冷氣
                                  	</c:if>
							</c:if>
							<c:if test="${! empty waterHeater}">
								<c:if test="${waterHeater == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasWaterHeater" value="true"
										checked>熱水器
                                  	</c:if>
							...
							<c:if test="${! empty internet}">
								<c:if test="${internet == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasInternet" value="true" checked>網路
                                  	</c:if>
							...
							<c:if test="${! empty fourthTV}">
								<c:if test="${fourthTV == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasFourthTV" value="true" checked>第四台
                                  	</c:if>
							...
							<c:if test="${! empty gas}">
								<c:if test="${gas == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasGas" value="true" checked>天然瓦斯
                                  	</c:if>
								<c:if test="${gas == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasGas" value="true">天然瓦斯
                                  	</c:if>
							</c:if>
							<c:if test="${! empty wardrobe}">
								<c:if test="${wardrobe == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasWardrobe" value="true" checked>衣櫃
                                  	</c:if>
								<c:if test="${wardrobe == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasWardrobe" value="true">衣櫃
                                  	</c:if>
							</c:if>
							<c:if test="${! empty sofa}">
								<c:if test="${sofa == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasSofa" value="true" checked>沙發
                                  	</c:if>
								<c:if test="${sofa == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasSofa" value="true">沙發
                                  	</c:if>
							</c:if>
							<c:if test="${! empty table}">
								<c:if test="${table == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasTable" value="true" checked>桌子
                                  	</c:if>
								<c:if test="${table == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasTable" value="true">桌子
                                  	</c:if>
							</c:if>
							<c:if test="${! empty chair}">
								<c:if test="${chair == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasChair" value="true" checked>椅子
                                  	</c:if>
								<c:if test="${chair == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasChair" value="true">椅子
                                  	</c:if>
							</c:if>
							<c:if test="${! empty parking}">
								<c:if test="${parking == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasParking" value="true" checked>停車位
                                  	</c:if>
								<c:if test="${parking == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasParking" value="true">停車位
                                  	</c:if>
							</c:if>
							<c:if test="${! empty balcony}">
								<c:if test="${balcony == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasBalcony" value="true" checked>陽台
                                  	</c:if>
								<c:if test="${balcony == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasBalcony" value="true">陽台
                                  	</c:if>
							</c:if>
							<c:if test="${! empty singleBed}">
								<c:if test="${singleBed == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasSingleBed" value="true" checked>單人床
                                  	</c:if>
								<c:if test="${singleBed == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasSingleBed" value="true">單人床
                                  	</c:if>
							</c:if>
							<c:if test="${! empty doubleBed}">
								<c:if test="${doubleBed == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasDoubleBed" value="true" checked>雙人床   
                                  	</c:if>
								<c:if test="${doubleBed == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="hasDoubleBed" value="true">雙人床   
                                  	</c:if>
							</c:if>
							<br>
							...
							<c:choose>
								<c:when
									test="${peopleNumGender == '1男' || peopleNumGender == '1女'}">
									<span id="ageDiv"> <input type="number" name="age"
										id="age" value="${age}" style="width: 80px;" min="18" max="99">歲
									</span>
								</c:when>
								<c:otherwise>
									<c:if
										test="${peopleNumGender == '1男1女' || peopleNumGender == '2男' || peopleNumGender == '2女' }">
										<span id="ageDiv" style="display: none"> <input
											type="number" name="age" id="age" style="width: 80px;"
											min="18" max="99">歲
										</span>
									</c:if>
								</c:otherwise>
							...
							<c:choose>
								<c:when
									test="${peopleNumGender == '1男1女' || peopleNumGender == '2男' || peopleNumGender == '2女'}">
									<span id="ageRange"> <input type="number" name="ageMin"
										id="ageMin" value="${ageMin}" style="width: 80px;">歲到
										<input type="number" name="ageMax" id="ageMax"
										value="${ageMax}" style="width: 80px;">歲
									</span>
								</c:when>
								<c:otherwise>
									<span id="ageRange" style="display: none"> <input
										type="number" name="ageMin" max="99" min="18"
										style="width: 80px;">歲到 <input type="number"
										name="ageMax" max="99" min="18" style="width: 80px;">歲
									</span>
								</c:otherwise>
							</c:choose>
							...
							<c:if test="${! empty agreeAdCondition}">
								<c:if test="${agreeAdCondition == true}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="agreeAdCondition" value="true"
										checked>是否同意成為廣告的搜索條件<br>
									<br>
								</c:if>
								<c:if test="${agreeAdCondition == false}">
									<input style="width: 30px; height: 20px; vertical-align: sub;" type="checkbox" name="agreeAdCondition" value="true">是否同意成為廣告的搜索條件<br>
									<br>
								</c:if>
							</c:if>
					...
					<td>
						...
						<fieldset id="wantedRoommates">
						  <legend style="font-size: 30px;color:#fff;background:#59ABE3;padding:10px;">您希望的室友</legend>
							...
							年齡: <select name="roommatesAgeMin" id="roommatesAgeMin"
								style="width: 80px;">
								<option value="default">-</option>
							</select>歲到 <select name="roommatesAgeMax" id="roommatesAgeMax"
								style="width: 80px;">
								<option value="default">-</option>
							</select>歲 <br>
							<br> 職業:
				...
				<tr>
					<td>
						<fieldset id="adTitle">
							...
							<br> 廣告標題: <input type="text" name="adTitle" required
								value="${adTitle}"><br>
							<br> (簡短的介紹)<br>
							<br> 現況描述:
							<textarea name="adDescription" rows="10" cols="50" required>${adDescription}</textarea>
							<br>
							<br> （描述中不允許使用聯繫方式)<br>
							<br>
							...
							<!--                                     </div><br><br> -->
							電話: <input type="tel" name="phone" name="phone"
								pattern="[0-9]{4}[0-9]{6}" placeholder="e.g. 0912345678"
								value="${phone}">
							(我們不會在4u4u上顯示您的號碼或將其傳遞給任何第三方。如果我們需要就您的帳戶與您聯繫或幫助驗證您的詳細信息，我們需要您的號碼)<br>
							<br> 上傳圖片: <br>
							<br>
							<div id="choosePicZone" style="clear: both">
								<input type="file" style="width: 200px;" name="imagefile"
									value="upload-photo" id="theFile"
				...
				<tr>
					<td>
						...
						<br><input id="updateButton" type="submit" value="修改廣告" style="width: 120px; height: 60px;font-size:22px;padding:0px;font-weight:bold" class="btn btn-warning text-center">
						<p>&nbsp;</p>
						<p>&nbsp;</p>
